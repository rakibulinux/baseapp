...
class IdentityComponent extends React.Component<Props, IdentityState> {
    constructor(props: Props) {
        super(props);
        const profile = props.user.profiles ? { ...props.user.profiles[props.user.profiles.length - 1] } : null;
        this.state = {
    ...
    private sendData = () => {
        ...
        if (!isIdentity && user.profiles && user.profiles.length > 0) {
            const isRejected = labels.find(el => el.key === 'profile' && el.value === 'rejected');
            if (isRejected) {
